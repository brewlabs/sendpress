...
class SendPress_View_Settings_Shared extends SendPress_View_Settings {
	...
	function save($post, $sp){
		...
		SendPress_Option::set('socailicons', $links );
		SendPress_Option::set('socailsize', $_POST['icon-view'] );
...
<p class="lead">Socail Icons appear in emails in Alphabetical order. If you enter a url in the box below then that icon will show in your emails.</p>
</div>
<div class="sp-50">
<p>
<label >
<input type="radio" name="icon-view" value="large" <?php checked( $socailsize, 'large' ); ?> /> Large ( 32px x 32px )
</label>
<br>
<label >
<input type="radio" name="icon-view" value="small" <?php checked( $socailsize, 'small' ); ?> /> Small ( 16px x 16px )
</label>
<br>
<label >
<input type="radio" name="icon-view" value="text" <?php checked( $socailsize, 'text' ); ?> /> Text
</label>
</p>
</div>
</div>
...
<?php 
  ...
  </div><input type="text" name="url-<?php echo $key; ?>" value="<?php echo $xlink; ?>" class="form-control" placeholder="URL To Social Site.." />
    </div>
		<?php
