...
function html($sp){
	...
	?>
	<link rel="stylesheet" href="<?php echo SENDPRESS_URL ?>css/uikit.css" />

	<link rel="stylesheet" href="<?php echo SENDPRESS_URL ?>css/addons/uikit.gradient.addons.css" />
	<script src="<?php echo SENDPRESS_URL ?>js/uikit.min.js"></script>
	...
	<script src="<?php echo SENDPRESS_URL ?>codemirror/mode/markdown/markdown.js"></script>
	<script src="<?php echo SENDPRESS_URL ?>codemirror/addon/mode/overlay.js"></script>
	<script src="<?php echo SENDPRESS_URL ?>codemirror/mode/xml/xml.js"></script>
	<script src="<?php echo SENDPRESS_URL ?>codemirror/mode/gfm/gfm.js"></script>
	<script src="<?php echo SENDPRESS_URL ?>js/marked.js"></script>

	<!-- HTML editor JavaScript -->
	<script src="<?php echo SENDPRESS_URL ?>js/addons/htmleditor.js"></script>
	<style>
	...
	s are NOT created automatically, so you need to wrap the table in one to use features like bulk actions -->
	<?php 
...
<textarea  data-uk-htmleditor="{mode:'tab', height: '750'}" id="template-content" name="template-content"><?php echo trim($template->post_content); ?></textarea>
...
*//*
		...
		*/
		//data-uk-htmleditor="{mode:'tab', height: 'auto'}"
		//var htmleditor = $.UIkit.htmleditor("#template-content", );
		var x = $( window ).height(); //jQuery('#wpbody').height();
		//$('.btn-group .btn').tooltip({container: 'body'});
		


		//myCodeMirror.setSize( $('.sp-75').width() ,(x - 350));
	});
</script>
...
<div class="sp-25 ">
	...
	<div  class="btn-toolbar">
		...
		{sp-content-area-one} : Main Content
		<hr>
		{sp-social-links} : Social Icons
		<hr>
		{sp-browser-url} : View in Browser Url 
